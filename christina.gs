...
var Christina = (function (ct) {
   ...
   */
    // h
  var hScript = (event) => {
      LineHelpers.replyBtnTemp(event.replyToken, '歡迎雇用 Christina', Christina.getCommandTemp())
    }

  // cmd
  var cmdScript = (event) => {
    if (getLineStatus()) {
      LineHelpers.replyMsg(event.replyToken, Christina.getCommandList(event.isMaster));
    }
  }
