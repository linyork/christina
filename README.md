...
- **主動狀態同步 (Proactive State Sync)**：即使 AI 決定「保持安靜 (Silent)」，也能在背景偷偷更新主人的狀態（如 `busyness: high`），確保下次醒來時更識相。

### 🛡️ 記憶優化與去重 (Memory Deduplication)

- **知識黑洞防護**：寫入知識庫前會自動進行語意包含檢查，防止重複資訊（如上班地點）不斷堆疊。
- **嚴格分類**：強制 AI 對長期記憶進行 Tag 分類（如 `work`, `health`），避免全部擠在 `auto_learned` 標籤中。
- **上下文無縫接軌**：在一般回覆 (Reply) 時，也會預載入使用者的作息與基本知識，消除「主動時很懂，被動時失憶」的割裂感。
